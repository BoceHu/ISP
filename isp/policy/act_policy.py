...
from isp.model.detr.main import (
...
class ACTPolicyWrapper(BaseImagePolicy):
    ...
    def predict_action(
        ...
        qpos = torch.cat(
            [
                nobs_dict["robot0_eef_pos"],
                nobs_dict["robot0_eef_quat"],
                nobs_dict["robot0_gripper_qpos"],
            ],
            dim=1,
        )
        image = nobs_dict["robot0_eye_in_hand_image"].unsqueeze(dim=1)
    ...
    def compute_loss(self, batch):
        ...
        forward_dict = self.model(
            ...
            image,
            nactions,
            torch.zeros([*nactions.shape[:2]]).bool().to(self.device),
        )
