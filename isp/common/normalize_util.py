from isp.model.common.normalizer import SingleFieldLinearNormalizer
from isp.common.pytorch_util import dict_apply, dict_apply_reduce, dict_apply_split
import numpy as np
...
def real_abs_action_symmetric_normalizer_from_stat(stat):
    result = dict_apply_split(
        stat, lambda x: {"pos": x[..., :3], "rot": x[..., 3:9], "gripper": x[..., 9:]}
    )
...
def robomimic_abs_action_only_so2_symmetric_normalizer_from_stat(stat):
    result = dict_apply_split(stat, lambda x: {"pos": x[..., :3], "other": x[..., 3:]})
